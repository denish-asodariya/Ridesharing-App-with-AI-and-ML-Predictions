...
const UserMessage = () => {
    ...
    const [rideInfo, setRiderInfo] = useState()
    ...
    const  handleCommunication = () => {
        navigator.clipboard.writeText(rideInfo?.driver?.phone)
        message.success(`Phone number copied successfully, ${rideInfo?.driver?.phone}`)
    }
    ...
    useEffect(()=> {
        if(rideInfo?.driver?._id) {
           getMessages(rideInfo?.driver?._id)
        }
    },[rideInfo?.driver?._id])
            ...
            <div className={'w-full md:w-2/3 p-3 bg-white rounded-lg mx-auto h-[90vh] md:h-[85vh] grid content-between'}>
                ...
                <div className={'flex items-center justify-between border-b-2 pb-2 border-gray-100'}>
                    <div className={'flex items-center gap-3'}>
                        <img src={rideInfo?.driver?.image} alt="rider-image" className={'w-14 h-14 rounded-full p-1 ring'} />
                        <p className={'text-gray-700 font-semibold tracking-wider capitalize'}>{rideInfo?.driver?.name}</p>
                    </div>
                ...
                <div className={'h-[60vh] overflow-y-scroll chatting_scrollbar'} ref={ref}>
                    {
                        (chatMsg || chatMsgAPI)?.map((data, i) => {
                            return <div key={data?._id}>
                                {
                                    (rideInfo?.user?._id !== data?.from?._id) ?
