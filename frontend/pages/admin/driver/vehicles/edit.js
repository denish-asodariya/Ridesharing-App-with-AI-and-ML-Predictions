...
const EditVehicle = () => {
    ...
    // load VEHICLE data
    useEffect(() => {
        fetchDriverVehicle({_id: query?._id}).then(res => {
            if (res?.error === false) {
                setVehicle(res?.data);
            }
        })
        ...
        <div>
            ...
            <Card className='my-3'>
                ...
                >
                    ...
                    <h1 className={'py-3 font-semibold text-14'}>Update documents : <span className={'text-cyan-600' +
                        ' underline cursor-pointer tracking-wider'}
                        onClick={() => push("/admin/driver/vehicles/document-update?driver=" + query?.driver)}>click here</span>
                    </h1>
