...
const DocumentUpdate = () => {
  ...
  const [formFields, getFormFields, { loading, error }] =
    useFetch(fetchFormFields);
  ...
  const handleImage = async (e, key) => {
    ...
    if (
      files.includes(e.target?.files[0]?.type) &&
  ...
  const onFinish = async (values) => {
    ...
    images?.forEach(
      (imag) => (values[Object.keys(imag)[0]] = Object.values(imag)[0])
    );
  ...
  return (
    <section className="container !mt-10">
      <span className="mb-4 ml-5 inline-block border-b border-main text-[16px] font-medium">
        Vehicle Information
      </span>
      ...
      <div className="px-3">
        ...
        >
              ...
              <>
                {checkFormType(field, formFields) === "text" && (
                  <FormInput
                    ...
                    placeholder={capitalizeFirstLetter(
                      field?.placeholder?.split("_").join(" ")
                    )}
                    name={field?.key}
                ...
                {checkFormType(field, formFields) === "number" && (
                  <FormInput
                    label={field?.key?.split("_").join(" ").toUpperCase()}
                    placeholder={capitalizeFirstLetter(
                      field?.key?.placeholder?.split("_").join(" ")
                    )}
                    name={field?.key}
            ...
            {driverDocuments?.map((field) => (
              <>
                {checkFormType(field, formFields) === "image" && (
                  ...
                  >
                    <ImageFromInput
                      onChange={(e) => handleImage(e, field?.key)}
              ...
              <>
                {checkFormType(field, formFields) === "textarea" && (
                  <FormTextArea
                    label={field?.key?.split("_").join(" ").toUpperCase()}
                    formName={field?.key}
                    formPlaceholder={capitalizeFirstLetter(
                      field?.key?.placeholder?.split("_").join(" ")
                    )}
          ...
          <div className="mt-3">
            {driverDocuments?.map((field) => (
              <>
                {checkFormType(field, formFields) ===
          ...
          <div className="flex gap-2 mb-4 mt-5">
            <button className="bg-main rounded-md text-twContent px-4 py-2 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg">
              Update
            </button>
          </div>
...
DocumentUpdate.layout = HomeLayout;
