...
import {objectToKeyValuePair} from "../../common/utilities";
...
const VehicleRegisterStepTwo = ({checkFieldStatus, formFieldsData = [], formData, vehicleParentsIds, setIsComplete, setToggle}) => {
    const i18n = useI18n()
    ...
    const onFinish = async (values) => {
        ...
        if ((checkFieldStatus('digital_signature') === (false || undefined) ? true : signature) && (values?.terms_and_conditions === false ? false : true)) {
            ...
            postDriverVehicle(details).then(res => {
                if (res?.error === false) {
                    toast.success(res?.msg)
                    setTimeout(async () => {
                        setIsComplete(pre => pre = true)
                    }, 3500);
                } else {
                    toast.error(res?.msg)
                }
            ...
            <div className='px-3'>
                ...
                >
                                ...
                                <>
                                    {
                                        ...
                                        <FormInput
                                            label={!!i18n && i18n.t(field?.input_name?.split('_').join(' ').toUpperCase())}
                                            placeholder={!!i18n && i18n.t(capitalizeFirstLetter(field?.placeholder?.split('_').join(' ')))}
                    ...
                    <div className='flex gap-2 mb-4 mt-5'>
                        ...
                        <Button onClick={handleBackPage} className='bg-gray-500 text-white'>BACK</Button>
...
export default VehicleRegisterStepTwo;