...
import {fetchUserExpenseGraphData} from "../../../helpers/backend_helper";

const Chart = dynamic(() => import('../../charts/ApexChart/chart'), {ssr: false})

const UserExpenseChart = ({chartTitle}) => {
    const [getReportData, setReportData] = useFetch(fetchUserExpenseGraphData);
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && getReportData?.length > 0) {
            setSeries([
                {
                    name: 'Expense',
                    data: getReportData?.map((d) => d?.expense_amount),
                },
            ]);
